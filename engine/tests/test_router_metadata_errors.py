...
def test_create_schema_error():
    ...
    response = client.post("/schemas/", json={"entity_name": "ErrorEntity", "fields": []})
...
def test_add_field_error():
    ...
    response = client.post("/schemas/TestEntity/fields", json={"name": "f1", "field_type": "string"})
    assert response.status_code == 400
...
def test_remove_field_error():
    ...
    response = client.delete("/schemas/TestEntity/fields/f1")
    assert response.status_code == 400
...
def test_update_schema_error():
    ...
    response = client.put("/schemas/TestEntity", json={"entity_name": "TestEntity", "fields": []})
    assert response.status_code == 400
...
def test_partial_update_schema_error():
    ...
    response = client.patch("/schemas/TestEntity", json={"description": "new"})
    assert response.status_code == 400
...
def test_update_field_error():
    ...
    response = client.put("/schemas/TestEntity/fields/f1", json={"name": "f1", "field_type": "number"})
    assert response.status_code == 400
...
def test_delete_schema_error():
    ...
    response = client.delete("/schemas/TestEntity")
    assert response.status_code == 400
